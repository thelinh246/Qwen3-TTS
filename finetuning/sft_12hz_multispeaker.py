# coding=utf-8
# Experimental multi-speaker Vietnamese fine-tuning for Qwen3/Gwen-TTS.
# This keeps Qwen's per-sample speaker_encoder(ref_mels) training behavior,
# but saves multiple custom_voice speaker slots into talker.model.codec_embedding.weight.
...
from dataset_multispeaker import TTSDataset
...
def train():
    ...
    parser.add_argument("--output_model_path", type=str, default="output_ms_vi")
    parser.add_argument("--train_jsonl", type=str, required=True)
    parser.add_argument("--batch_size", type=int, default=2)
    parser.add_argument("--lr", type=float, default=2e-6)
    parser.add_argument("--num_epochs", type=int, default=3)
    parser.add_argument("--speaker_key", type=str, default="channel", help="JSONL field used as speaker name. For viVoice use channel; for custom data use speaker.")
    parser.add_argument("--speaker_id_start", type=int, default=3000)
    parser.add_argument("--speaker_slot_idx", type=int, default=6, help="Official Qwen finetuning layout uses speaker slot 6.")
    parser.add_argument("--save_interval", type=int, default=1)
    parser.add_argument("--save_last/--no-save_last", dest="save_last", default=True, action=argparse.BooleanOptionalAction)
    parser.add_argument("--log_interval", type=int, default=10)
    parser.add_argument("--logging_dir", type=str, default=None)
    parser.add_argument("--resume/--no-resume", dest="resume", default=False, action=argparse.BooleanOptionalAction)
    parser.add_argument("--accelerate-trackers/--no-accelerate-trackers", dest="accelerate_trackers", default=False, action=argparse.BooleanOptionalAction)
    parser.add_argument("--flash-attn/--no-flash-attn", dest="flash_attn", default=True, action=argparse.BooleanOptionalAction)
    parser.add_argument("--lr_scheduler", type=str, default="cosine", choices=["cosine", "linear", "constant", "constant_with_warmup"])
    parser.add_argument("--warmup_steps", type=int, default=0)
    parser.add_argument("--warmup_ratio", type=float, default=0.03)
    parser.add_argument("--language", type=str, default="Vietnamese")
    parser.add_argument("--normalize_text/--no-normalize_text", dest="normalize_text", default=True, action=argparse.BooleanOptionalAction)
    parser.add_argument("--freeze_text_understanding/--no-freeze_text_understanding", dest="freeze_text_understanding", default=True, action=argparse.BooleanOptionalAction)
    parser.add_argument("--freeze_speaker_encoder/--no-freeze_speaker_encoder", dest="freeze_speaker_encoder", default=True, action=argparse.BooleanOptionalAction)
    ...
    if args.accelerate_trackers:
        try:
            ...
            accelerator = Accelerator(gradient_accumulation_steps=grad_accum_steps, mixed_precision="bf16")
    else:
        accelerator = Accelerator(gradient_accumulation_steps=grad_accum_steps, mixed_precision="bf16")

    tb_writer = None
    if SummaryWriter is not None and accelerator.is_main_process:
        tb_writer = SummaryWriter(log_dir=str(Path(args.output_model_path) / "tb"))
    ...
    if config_path.exists():
        with open(config_path, "r", encoding="utf-8") as f:
            cfg = json.load(f)
        # custom_voice checkpoint is loaded as base during further training.
    ...
    def _speaker_name(item):
        for key in (args.speaker_key, "speaker", "channel"):
            if key in item and item[key]:
                return str(item[key]).strip()
        raise ValueError(f"Missing speaker field. Need '{args.speaker_key}', 'speaker', or 'channel'. Item keys: {list(item.keys())}")
    ...
    for item in train_data:
        ...
        item["speaker"] = _speaker_name(item)  # normalize to canonical key for dataset/inference
        ...
        # Choose one stable reference audio per speaker for the saved custom_voice slot.
        # If your JSONL already uses a clean per-speaker ref_audio, this is exactly what we want.
        spk = item["speaker"]
        if spk not in speaker_ref_audio:
            speaker_ref_audio[spk] = item.get("ref_audio") or item.get("audio")

    speaker_names = sorted({item["speaker"] for item in train_data})
    if not speaker_names:
        raise ValueError("No speakers found in train_jsonl.")
    speaker_to_id = {name: args.speaker_id_start + i for i, name in enumerate(speaker_names)}
    counts = Counter(item["speaker"] for item in train_data)

    accelerator.print("Speakers:")
    for name in speaker_names:
        accelerator.print(f"  {name}: id={speaker_to_id[name]}, samples={counts[name]}, ref={speaker_ref_audio[name]}")

    dataset = TTSDataset(train_data, qwen3tts.processor, config, speaker_key="speaker")
    ...
    def _compute_saved_speaker_embeddings(unwrapped_model):
        """Compute one stable embedding per speaker from speaker_ref_audio on the main process."""
        was_training = unwrapped_model.training
        unwrapped_model.eval()
        out = {}
        device = accelerator.device
        dtype = next(unwrapped_model.parameters()).dtype
        for spk in speaker_names:
            ref_path = speaker_ref_audio[spk]
            wav, sr = dataset._load_audio_to_np(ref_path)
            ref_mel = dataset.extract_mels(wav, sr).to(device=device, dtype=dtype)
            emb = unwrapped_model.speaker_encoder(ref_mel).detach()[0].cpu()
            out[spk] = emb
        if was_training:
    ...
    def _save_checkpoint(epoch_idx: int):
        ...
        talker_config["spk_id"] = speaker_to_id
        talker_config["spk_is_dialect"] = {name: False for name in speaker_to_id}
        ...
        if weight_key not in state_dict:
            raise KeyError(f"Cannot find {weight_key} in model state_dict.")
        weight = state_dict[weight_key]
        max_needed = max(speaker_to_id.values())
        if max_needed >= weight.shape[0]:
            raise ValueError(f"Need speaker id up to {max_needed}, but codec embedding only has {weight.shape[0]} rows.")

        saved_embeddings = _compute_saved_speaker_embeddings(unwrapped_model)
        for spk, spk_id in speaker_to_id.items():
            weight[spk_id] = saved_embeddings[spk].to(device=weight.device, dtype=weight.dtype)
        state_dict[weight_key] = weight

        # Optional: keep speaker_encoder to allow resume/debug. Remove these keys only if you want a smaller inference-only checkpoint.
        save_file(state_dict, os.path.join(output_dir, "model.safetensors"))

        with open(os.path.join(output_dir, "speaker_map.json"), "w", encoding="utf-8") as f:
            json.dump({"speaker_to_id": speaker_to_id, "speaker_ref_audio": speaker_ref_audio}, f, indent=2, ensure_ascii=False)
            ...
            json.dump(
                {
                    ...
                    "speaker_slot_idx": args.speaker_slot_idx,
                    "freeze_text_understanding": args.freeze_text_understanding,
    ...
    for epoch in range(start_epoch, num_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                ...
                input_codec_embedding[:, args.speaker_slot_idx, :] = speaker_embedding
